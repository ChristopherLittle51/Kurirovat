...
import {
  ...
  CondensedTextSchema,
  createContentStrategySchema,
  createDraftSchema,
  createEvidenceResolutionSchema,
  IdealJobSchema,
...
async function matchEvidence(
  ...
  const result = await runStructured({
    ...
    schema: createEvidenceResolutionSchema(Array.from(references.refToId.keys())),
...
async function createStrategyAndDraft(args: {
  ...
  if (!strategy) {
    const strategyResult = await runStructured({
      ...
      schema: createContentStrategySchema(Array.from(references.refToId.keys())),
  ...
  if (!draft) {
    const draftResult = await runStructured({
      ...
      schema: createDraftSchema(Array.from(references.refToId.keys())),
