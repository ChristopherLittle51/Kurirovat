...
4. `supabase/functions/gemini-api` handles `processGenerationJob` idempotently, advances one model stage, checkpoints `working_state`, and returns queued work for the application scheduler. It inserts the final `applications` row and marks the job `succeeded` only after all stages finish.
