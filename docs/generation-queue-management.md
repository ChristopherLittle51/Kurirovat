...
Run `pnpm build`. Manually verify: submit a new application, leave the form, observe progress at `/admin/generation-queue`, retry a failed job, cancel an active job, answer an evidence question, open a successful application, and remove terminal history. Do not claim database enforcement is deployed until the new migration has been applied to the target Supabase project.
