...
const TemplateSwitcher: React.FC<Props> = ({ title = "Choose Template", currentTemplate, onSelect }) => {
    ...
    return (
        <div className="w-full min-w-0 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-4 transition-colors">
            {/* Header */}
            <div className="flex items-center justify-between gap-3 mb-4">
                <h3 className="min-w-0 text-lg font-bold text-gray-800 dark:text-gray-200 truncate">{title}</h3>
                ...
                <div className="grid grid-cols-1 min-[360px]:grid-cols-2 gap-3">
                    {filteredTemplates.map((template) => (
                        <button
                            ...
                            className={`relative group rounded-lg border-2 overflow-hidden transition-all hover:shadow-md min-w-0 ${currentTemplate === template.id
                        ...
                        >
                            ...
                            <div className="p-3 bg-white dark:bg-gray-900 transition-colors">
                                <div className="flex items-center justify-between gap-2 min-w-0">
                                    ...
                                    <span className={`shrink-0 text-[10px] px-1.5 py-0.5 rounded transition-colors ${getCategoryColor(template.category)}`}>
                                        {template.category}
                                    </span>
                                </div>
                            </div>
                ...
                <div className="space-y-2">
                    {filteredTemplates.map((template) => (
                        <button
                            key={template.id}
                            onClick={() => onSelect(template.id as TemplateId)}
                            className={`w-full min-w-0 flex items-center gap-3 p-3 rounded-lg border-2 transition-all ${currentTemplate === template.id
                        ...
                        >
                            ...
                            <div className="min-w-0 flex-1 text-left">
                                <div className="font-medium text-gray-800 dark:text-gray-200 transition-colors truncate">{template.name}</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400 transition-colors line-clamp-2">{template.description}</div>
                            </div>
