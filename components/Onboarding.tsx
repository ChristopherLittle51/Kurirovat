...
const Onboarding: React.FC<Props> = ({ onComplete, initialData }) => {
  ...
  const toggleRepo = (repo: GithubProject) => {
    const exists = profile.githubProjects?.find(r => r.id === repo.id);
    if (exists) {
      setProfile(prev => ({
        ...prev,
        githubProjects: prev.githubProjects?.filter(r => r.id !== repo.id)
      }));
    } else {
      setProfile(prev => ({
        ...prev,
        githubProjects: [...(prev.githubProjects || []), repo]
      }));
    }
  };
    ...
    <div className="max-w-2xl mx-auto py-12 px-4 transition-colors">
      ...
      {step === 1 && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {(availableRepos.length > 0 || (profile.githubProjects && profile.githubProjects.length > 0)) && (
            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Select Repositories for Portfolio</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-2">
                {/* Combine available repos with already selected ones (in case they aren't in the fetch result for some reason) */}
                {[...new Map([...availableRepos, ...(profile.githubProjects || [])].map(item => [item.id, item])).values()]
                  .sort((a, b) => b.stargazers_count - a.stargazers_count)
                  .map(repo => {
                    const isSelected = profile.githubProjects?.some(r => r.id === repo.id);
                    return (
                      <div
                        key={repo.id}
                      ...
                      >
                        <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 mt-0.5 ${isSelected ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-300 dark:border-gray-600'
                          }`}>
                          {isSelected && <Check size={14} />}
                        </div>
                        <div className="min-w-0">
                          <div className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{repo.name}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{repo.description || 'No description'}</div>
                          <div className="flex items-center gap-3 mt-1 text-xs text-gray-400 dark:text-gray-500">
                            {repo.language && <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-yellow-400/80"></span>{repo.language}</span>}
